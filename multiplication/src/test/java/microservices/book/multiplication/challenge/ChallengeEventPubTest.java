...
class ChallengeEventPubTest {
  ...
  void sendsAttempt(boolean correct) {
    ...
    // when
    challengeEventPub.challegeSolved(attempt);

    // then
    var exchageCaptor = ArgumentCaptor.forClass(String.class);
    ...
    verify(amqpTemplate)
        .convertAndSend(exchageCaptor.capture(), routingKeyCaptor.capture(), eventCaptor.capture());
    then(exchageCaptor.getValue()).isEqualTo("test.topic");
